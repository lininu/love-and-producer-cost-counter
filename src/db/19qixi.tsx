...
export const qAndA = [
  {
    ...
    description: `僅為計算結果，負數表示會多出許多鑽石+同心鎖 (在本計算機已將此全數轉為鑽石做計算)。本計算機以單次活動「課金取得卡片」為主要目標，因此較建議選擇結果為大於等於0的購買數量，或者不要先把禮盒拿到的鑽石換成同心鎖，避免同心鎖過剩。`
  },
  {
    title: `計算機結果「所需總同心鎖」看起來怪怪的？以2SSR為例，為什麼不是3688 * 2 = 7376？`,
    description: <span>本計算機用結果回推，在最後一天取卡，並且<b>四人的每日免費都有執行</b>，在最後一天的時候會逐張SSR完成(請參考此篇 <a href="https://www.facebook.com/groups/302104066944567/permalink/375891872899119/">Ting-Chieh Yen</a> 之計算結果&取卡步驟 )，「機率掉落同心鎖後，仍需同心鎖數」是按「該次數區間的占比」乘與「該次數區間加成」才扣除贈送額度。以2SSR為例，還會加入其他兩人的免費次數 (3688(一張SSR) * 2(目標2張)) + (5(免費) * 13(期間) * 2) = 7506。最後計算結果時，會把其他人的結果一併扣掉。</span>
  },
  {
    title: `如何執行可以最接近計算結果？`,
    description: `最後一天再拿4張SSR，並且先拿到第一張SSR合成後，再跑第二張SSR的流程，一樣先合成第二張SSR。因為在第一張SSR合成時，應該可以同時達成5%同心鎖掉落機率的條件，同理第二張SSR合成時，可以達成10%同心鎖掉落率，因此除了每日都要刷免費拍攝次數外，最後一天建議不要先把4人免費的拍攝次數刷完，而是逐人刷&合成完成任務。`
  },
  {
  ...
  {
    title: `為何「在第N次拍攝後開始有5%同心鎖掉落機率」預設值為「3883」？`,
    description: `這邊預期拿到第一張SSR的時候可以同時達成事件累計99次，並包含全部四人的免費次數：3688(1SSR所需次數) + ( 3(其他三人) * 13(活動天數-1) * 5(每日免費) )，按Q&A第三條所述，應該會在第3883次拍攝時得到5%同心鎖掉落率。`
  },
  {
