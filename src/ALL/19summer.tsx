...
class Summer extends Component<any, any> {
  ...
  reccount(isInit = false) {
    ...
    const countedDataSource = dataSource.map((data: any, index: number) => {
      data = {
        ...
        ['船票+鑽石(1元可換鑽)']: (data[CONST.ACTIVITY_ITEM] * itemCost + data[CONST.DIMAND]) / data[CONST.PRICE],
      };
      if (isInit) {
        data = {
          ...data,
      ...
      for (let i = 0; i < 5; i++) {
        for (let j = 4 - i; j >= 0; j--) {
          ...
          result = [...result, {
            ...
            ['其他r']: j,
            ['所需總船票']: 所需總船票,
            ['機率掉落船票後，仍需船票數']: 機率掉落船票後仍需船票數,
            ['扣除贈送額度後仍需船票數']: 扣除贈送額度後仍需船票數,
            ['所需鑽數']: 扣除贈送額度後仍需船票數 * itemCost,
            ['扣除禮盒後仍需船票數']: 扣除禮盒後仍需船票數,
            ['扣除禮盒後仍需鑽數']: 扣除禮盒後仍需船票數 * itemCost,
          }];
        }
      }
      return result;
              ...
              <table className="table table-striped">
                <tbody>
                  <tr className="bg-secondary text-white">
                    <th className="text-right">總船票數</th>
                    <td className="text-right">{numeral(_.sumBy(dataSource, '可獲總船票數')).format(NUMERAL_FORMAT)}</td>
                  </tr>
          ...
          <div className="col-12 col-lg-5">
            <div className="table-responsive">
              <table className="table table-striped">
                <tbody>
                  <tr>
                    <th className="text-right">官方贈送船票</th>
                    <td className="text-right">{gift}</td>
                  </tr>
                  <tr>
                    <th className="text-right">一張R卡需要船票數</th>
                    <td className="text-right">{perRNeedItemsCount}</td>
                  </tr>
                  <tr>
                    <th className="text-right">一張SSR卡需要船票數(附帶一張R卡)</th>
                    <td className="text-right">{perSSRNeedItemsCount}</td>
                  </tr>
                  <tr>
                    <th className="text-right">在第N次拍攝後開始有5%船票掉落機率</th>
                  ...
                  <tr>
                    <th className="text-right">在第N次拍攝後開始有10%船票掉落機率</th>
                  ...
                  <tr>
                    <th className="text-right">一張船票需要鑽石數</th>
